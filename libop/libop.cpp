...
void libop::GetScreenData(long x1, long y1, long x2, long y2, size_t* data, long* ret)
{
	...
	*ret = 0;
	...
	try {
		// 检查上下文和图像处理器是否有效
		if (!m_context || !m_context->image_proc._src.pdata || m_context->image_proc._src.width <= 0 || m_context->image_proc._src.height <= 0) {
			setlog("Invalid context or image source");
			return;
		}
		...
		if (m_context->bkproc.check_bind() && m_context->bkproc.RectConvert(x1, y1, x2, y2)) {
			...
			if (!m_context->bkproc.requestCapture(x1, y1, x2 - x1, y2 - y1, m_context->image_proc._src)) {
				...
				if (img_size == 0 || img.height <= 0 || img.width <= 0) {
					setlog("Invalid image size");
					return;
				}

				m_context->screenData.resize(img_size * 4);

				// 拷贝图像数据
				if (m_context->screen_data_mode == SC_DATA_BOTTOM) {
					for (int i = 0; i < img.height; i++) {
						// 防止越界访问
						if ((i * img.width * 4) < m_context->screenData.size() &&
							img.ptr<char>(img.height - 1 - i)) {
							memcpy(m_context->screenData.data() + i * img.width * 4,
								img.ptr<char>(img.height - 1 - i), img.width * 4);
						}
						else {
							setlog("Error in copying image data");
							return;
						}
					}
				}
					...
					// 防止越界访问
					if (img.pdata && img_size * 4 <= m_context->screenData.size()) {
						memcpy(m_context->screenData.data(), img.pdata, img_size * 4);
					}
					else {
						setlog("Error in copying image data");
						return;
			...
			setlog("check_bind or RectConvert failed");
		}
	}
	catch (const std::exception& e) {
		setlog((std::string("Exception: ") + e.what()).c_str());
	}
	catch (...) {
		setlog("Unknown exception occurred");
	}
}
