...
long OP_ClearDict(libop* _op, long idx)
{
    long ret;
    _op->ClearDict(idx, &ret);
    return ret;
}
...
int OP_FetchWord(libop* _op, long x1, long y1, long x2, long y2, const wchar_t* color, const wchar_t* word, wchar_t* _pStr, int _nSize)
{
    std::wstring retstr;
    _op->FetchWord(x1, y1, x2, y2, color, word, retstr);
...
int OP_FindPicS(libop* _op, long x1, long y1, long x2, long y2, const wchar_t* files, const wchar_t* delta_color, double sim, long dir, long* x, long* y, wchar_t* _pStr, int _nSize)
{
    std::wstring retstr;
    _op->FindPicS(x1, y1, x2, y2, files, delta_color, sim, dir, x, y, retstr);
    if (_pStr == nullptr || _nSize <= (int)(retstr.length() * sizeof(wchar_t)))
        return (int)(retstr.length() + 1) * sizeof(wchar_t);
    wcscpy_s(_pStr, _nSize / sizeof(wchar_t), retstr.c_str());
    return 0;
...
long OP_GetDictCount(libop* _op, long idx)
{
    long ret;
    _op->GetDictCount(idx, &ret);
    return ret;
}
